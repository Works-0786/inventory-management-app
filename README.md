...
- 配列に追加した要素を画面へ描画する `render()` を、どのタイミングで呼び出すべきか理解するのに苦労した。
  処理の流れ（追加→再描画）を意識して設計する重要性を学んだ。

- 商品IDの再採番処理で `Math.max()` を用いた際、空配列に対して実行すると `-Infinity` が返ることを知った。
  配列の要素数を確認してから処理を行うことで、例外を防ぐ「防御的な実装」の考え方を学んだ。
